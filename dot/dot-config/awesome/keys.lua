...
local volume_up = awful.key({modkey}, "F12", function() volume_widget:inc(5) end)
local volume_down = awful.key({modkey}, "F11", function() volume_widget:dec(5) end)
local default_launcher = awful.key({modkey}, "p", function() menubar.show() end, {description = "show the menubar", group = "launcher"})
local terminal = awful.key({modkey}, "Return", function() awful.spawn(terminal) end, {description = "open a terminal", group = "launcher"})
local app_launcher = awful.key({modkey}, "d", function() awful.spawn(launcher) end, {description = "application launcher", group = "launcher"})
-- {{{ Key bindings{{{
globalkeys = gears.table.join(
    ...
    awful.key({modkey, "Shift"}, "]", function() awful.client.focus.byidx(1) end, {description = "focus next by index", group = "client"}),
    awful.key({modkey}, "Left", function() awful.client.focus.byidx(-1) end, {description = "focus previous by index", group = "client"}),
    awful.key({modkey, "Shift"}, "[", function() awful.client.focus.byidx(-1) end, {description = "focus previous by index", group = "client"}),
    awful.key({modkey}, "w", function() mymainmenu:show() end, {description = "show main menu", group = "awesome"}), -- Layout manipulation
    ...
    awful.key({modkey, "Control"}, "[", awful.tag.viewprev, {description = "view previous", group = "tag"}),
    awful.key({modkey, "Control"}, "]", awful.tag.viewnext, {description = "view next", group = "tag"}), -- launch rofi
    ...
    terminal, awful.key({modkey}, "r", awesome.restart, {description = "reload awesome", group = "awesome"}),
    awful.key({modkey, "Shift"}, "End", awesome.quit, {description = "quit awesome", group = "awesome"}),
